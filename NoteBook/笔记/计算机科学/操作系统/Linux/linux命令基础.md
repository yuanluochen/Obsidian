...
### ls命令（-l -a -i -d -R）![](../../../../rescource/Picture/Pasted%20image%2020230708100347.png)

**ls**显示当前目录（文件夹）下的文件，如图所示。

![1.1.1](../../../../rescource/Picture/2022-07-09-09-37-43.png)

#### **-l选项**，显示当前目录下的文件部分属性信息

![1.1.2](../../../../rescource/Picture/2022-07-09-09-38-55.png)
...
![1.1.5](../../../../rescource/Picture/2022-07-09-16-47-16.png)
...
1. 首先生成一个可执行性文件test  
![1.1.6](../../../../rescource/Picture/2022-07-09-10-38-59.png)
2. 将可执行文件test进行更名为test.txt
![1.1.7](../../../../rescource/Picture/2022-07-09-11-08-42.png)
在Windows系统中将可执行性文件（.exe）更名为文本文件（.txt），文件不可执行，而在Linux系统中则可执行。
3. 执行test的更名文件test.txt
![1.1.8](../../../../rescource/Picture/2022-07-09-11-11-46.png)
文件的inode编号不变，文件的类型改变了
![1.1.9](../../../../rescource/Picture/2022-07-09-16-41-35.png)
文件依旧可如同可执行文件一样执行。
...
![1.1.9](../../../../rescource/Picture/2022-07-09-16-57-47.png)

若在 **ls -d** 后加特定文件名，即**ls -d 文件名**显示指定文件，如下图

![1.1.10](../../../../rescource/Picture/2022-07-09-17-00-08.png)

#### **-R选项**，显示当前目录下的子文件

如下图

![1.1.11](../../../../rescource/Picture/2022-07-09-17-12-37.png)

### pwd命令

**pwd** 命令：显示当前所处的绝对路径（当前位置），如下图

![1.2.1](../../../../rescource/Picture/2022-07-09-18-08-07.png)
...
![1.2.2](../../../../rescource/Picture/2022-07-08-22-12-00.png)
...
cd后接相对路径
![1.3.1](../../../../rescource/Picture/2022-07-09-20-21-44.png)
cd后接绝对路径  
![1.3.2](../../../../rescource/Picture/2022-07-09-20-20-00.png)
...
![1.4.1](../../../../rescource/Picture/2022-07-10-11-59-01.png)
...
![1.5.1](../../../../rescource/Picture/2022-07-10-12-37-21.png)
...
![1.8.1](../../../../rescource/Picture/2022-07-10-13-52-28.png)
若想删除输入 y ；不想删除输入 n
...
![1.9.1](../../../../rescource/Picture/2022-07-10-14-06-25.png)
...
![1.9.2](../../../../rescource/Picture/2022-07-10-14-11-07.png)

### man指令

查找命令、系统调用、库函数……的说明，如下图查找rm命令的说明

![1.10.1](../../../../rescource/Picture/2022-07-10-14-18-54.png)

回车之后，就会显示说明

当我们输入命令 **man man** 即查找man的说明我们发现 man 命令查找时有多个接口，如下图。
![1.10.2](../../../../rescource/Picture/2022-07-10-14-32-49.png)
...
![1.11.1](../../../../rescource/Picture/2022-07-11-13-41-36.png)

首先要明确一点，在同一个目录下，文件间的文件名是不可冲突的，若是拷贝文件到当前目录且不重命名，则会报错，如下图

![1.11.2](../../../../rescource/Picture/2022-07-11-13-45-35.png)

这时，若实在需要拷贝文件到原目录，则需对文件进行重命名。如下图所示。

![1.11.3](../../../../rescource/Picture/2022-07-11-13-49-23.png)

#### **-r**选项，对文件进行递归式的拷贝（主要针对拷贝目录）

若想拷贝目录，需要如同删除目录时一样（rm -r），对文件进行递归式拷贝 **cp -r**，即先拷贝其子目录，再一步步向上级文件进行拷贝。

![1.11.4](../../../../rescource/Picture/2022-07-11-14-30-28.png)
...
![1.12.1](../../../../rescource/Picture/2022-07-11-14-12-43.png)

#### -n选项，表示行号

如下图

![1.12.2](../../../../rescource/Picture/2022-07-11-21-59-50.png)

### tac命令

tac：相反于 **cat**命令方向查找文件内容，如下图

![1.12.3](../../../../rescource/Picture/2022-07-11-22-05-02.png)
...
![1.14.1](../../../../rescource/Picture/2022-07-11-15-11-21.png)
...
![1.14.2](../../../../rescource/Picture/2022-07-11-17-37-59.png)
...
![1.15.1](../../../../rescource/Picture/2022-07-11-18-03-37.png)
...
![1.16.1](../../../../rescource/Picture/2022-07-11-18-15-46.png)
...
![1.17.1](../../../../rescource/Picture/2022-07-11-22-08-19.png)
...
1. 输出年月日
   如图所示
   ![1.18.1](../../../../rescource/Picture/2022-07-11-22-18-06.png)
   在图中输出时间的固定格式为 **date +%** 切记加号与date之间必须要有空格
   **%Y**为输出年 **%m**代表输出月 **%d**代表输出日（Y、m、d需要区分大小写），“/”可以由自己随意更改，无具体格式
2. 输出时分秒
   如图所示
   ![1.18.2](../../../../rescource/Picture/2022-07-11-22-24-05.png)
...
![1.20.1](../../../../rescource/Picture/2022-07-11-22-41-33.png)

也可以输入 **cal 年份**查看该年份的日历信息

![1.20.2](../../../../rescource/Picture/2022-07-11-22-43-51.png)
...
1. 如图
   ...
   ![1.22.1](../../../../rescource/Picture/2022-07-11-22-52-00.png)

   若有方块内两项有时间，则证明联网

2. 或输入一个不存在的网址，如下图

   ![1.22.2](../../../../rescource/Picture/2022-07-11-22-58-20.png)
   若出现红框内的报错则证明联网。
...
![1.22.3](../../../../rescource/Picture/2022-07-11-23-12-46.png)

查看inet是否有地址，若无，则继续

输入命令 **sudo（若权限允许可不加） ifup ens33（与上图网络接口对应，其他机器可能不是ens33，可能为其他）**，如图

![1.22.4](../../../../rescource/Picture/2022-07-11-23-18-48.png)
...
![1.22.4](../../../../rescource/Picture/2022-07-11-23-27-31.png)
...
1. 切换至高级用户root
   1. 输入命令 **su root**，如下图
   ![1.23.1](../../../../rescource/Picture/2022-07-12-13-21-01.png)
   2. 输入命令 **su**
2. 切换至普通用户
   输入命令 **su 普通用户名**
